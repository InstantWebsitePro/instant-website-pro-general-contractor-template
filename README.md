# Instant Website Pro General Contractor Publishing System V8

This repository is the publishing and safety foundation for the **desktop Instant Website Pro program for general contractors**. It validates public output, protects the private/public boundary, packages approved websites, supports Cloudflare Pages publishing and keeps a restore path. The owner supplies business knowledge and reviews the result; the program handles the website work.

Dedicated template: https://github.com/InstantWebsitePro/instant-website-pro-general-contractor-template
...
Program content is V8; the tested schema and transport contract remains **6.0**. Use `business_type: general-contractor` with `contractor_profile.trade_categories: ["general-contracting-remodeling"]` for this edition's generated public manifest. Keep the existing nested fields and enums. Use truthful `GeneralContractor` structured data with only supported public facts; a valid schema is not a search-ranking promise.
